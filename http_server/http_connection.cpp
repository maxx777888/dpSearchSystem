...
void HttpConnection::createResponsePost()
{
	if (request_.target() == "/")
	{
		...
		for (const auto& url : searchResult)
		{
			...
			if (url == "This_is_warning")
			{
				beast::ostream(response_.body())
					<< "<p style=\"color: red\">Warning: It was not possible to establish a connection with the DB.</p>\n";
			}
			else if (url == "NO_RESULTS") {
				beast::ostream(response_.body())
				<< "<p style=\"color: orange\">No results were found for your search query</p>\n";
			}
			else {
				beast::ostream(response_.body())
					<< "<li><a href=\""
